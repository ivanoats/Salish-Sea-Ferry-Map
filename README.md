...
The basemap selector includes [Stamen Terrain from Stadia Maps](https://docs.stadiamaps.com/map-styles/stamen-terrain/), using retina raster tiles with provider attribution. To use an API key, set `NEXT_PUBLIC_STADIA_MAPS_API_KEY` in `.env.local` and restart the development server. This key is included in the browser bundle and tile requests; never commit it. For deployment, either set the variable in the hosting provider's build environment and rebuild, or leave it unset and register the site's domain with Stadia Maps. Stadia recommends [domain authentication](https://docs.stadiamaps.com/authentication/) for production websites and allows keyless localhost development with stricter rate limits.
