...
Coordinates are dock-level approximations. Route legs follow baked geometry through navigable water (`src/data/route-leg-geometry.ts`, generated by `npm run build-route-geometry` from the vendored mesh in `data/`), falling back to a straight line where the mesh cannot serve a terminal pair. That geometry follows the middle of navigable water as OSM describes the shoreline; it carries no soundings and clears no rocks. **This map is not for navigation.**
...
```bash
npm run build-route-geometry   # re-bake src/data/route-leg-geometry.ts from data/salish-mesh.json
npm run test        # vitest
