...
It reads the same route records as the map. The only extra data is `src/data/schematic-layout.ts`, which gives each terminal a grid position, a short label, and which side the label sits on. When you add a terminal, place it there as well: `src/data/__tests__/schematic-layout.test.ts` fails until every terminal has a place, and it also fails if a leg leaves the 45° grid or runs through a terminal it doesn't stop at. Lane order through shared corridors is worked out in `src/domain/schematic.ts`: each pair of routes is followed to where they part, so lines don't cross inside a bundle. See [ADR 0006](./docs/adr/0006-octolinear-diagram-as-separate-page.md).
